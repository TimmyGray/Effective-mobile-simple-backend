...
| B-L3 | Split `accounts/tests.py` into `accounts/tests/` package (per-class modules or domain groupings) | backend | 0.5d | todo | ~886 lines; navigation and parallel CI benefit; repeatedly deferred in sweep notes |
| B-L4 | Upgrade pip in dev/CI to ≥26.0.1 (CVE-2026-1703); optionally add `pip-audit` to `/validate` or CI advisory step | infra | 0.1d | todo | `pip-audit` clean for Django/DRF pins; vuln was tooling-only on audited env |
...
| FEAT-3 | Improve workflow for manual API checks (curl/httpie): documented steps, CSRF cookie/header, 401 vs 403 probes, optional small script under `docs/` or `scripts/` | workflow | 0.25d | done | Merged PR #14; `docs/manual-api-checks.md` + `scripts/probe_auth_semantics.py` |
